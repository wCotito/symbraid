...
Controlled-прогон на закоммиченном polyglot fixture (шесть запросов с ручной
оценкой, один исключённый warm-up и пять измеряемых CLI invocation, Windows 11 x64) дал первый regression baseline:

| Метрика | Symbraid 0.3.0 | ripgrep 15.2.0 |
| --- | ---: | ---: |
| nDCG@10 | 0.830 | 0.634 |
| Recall@10 | 0.886 | 0.622 |
| Warmed CLI invocation p50 | 5 435 мс | 52 мс |

На этом небольшом fixture Symbraid лучше ранжировал семантические результаты, а
ripgrep оказался значительно быстрее. Это частичное сравнение, а не
универсальное заявление или полный рейтинг конкурентов. Cold indexing и
ресурсные метрики не измерялись; изолированные прогоны Codanna,
open-codebase-index и Zilliz Claude Context с зафиксированными версиями ещё
предстоит выполнить. Полная таблица, оборудование, ограничения, provenance
hashes и команды воспроизведения находятся в [benchmark-отчёте](benchmarks.md).
## Документация
