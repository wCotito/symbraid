...
class ManagePanel {
  ...
  async open(folder) {
    ...
    if (!this.panel) {
      ...
      this.panel = vscode.window.createWebviewPanel('symbraidManage', 'Symbraid: Manage', vscode.ViewColumn.Active, {
        enableScripts: true,
  ...
  async handleMessage(message) {
    ...
    try {
      let result;
      if (message.command === 'ready' || message.command === 'reload') return this.sendState();
      if (message.command === 'choosePath') {
        const picked = await vscode.window.showOpenDialog({
          canSelectFiles: message.kind === 'file',
          canSelectFolders: message.kind !== 'file',
          canSelectMany: false,
          defaultUri: vscode.Uri.file(message.value || cwd),
          openLabel: this.locale === 'ru' ? 'Выбрать' : 'Select',
        });
      ...
      if (message.command === 'saveDefaults') {
        ...
        this.panel.webview.postMessage({ type: 'plan', plan: result, payload: message.payload });
        return;
      } else if (message.command === 'applyProject') {
        const impact = message.payload.impact || 'configuration-only';
        const warning = this.locale === 'ru'
          ? `Применить изменения (${impact})? Предыдущий source будет сохранён.`
          : `Apply changes (${impact})? The previous source will be retained.`;
        ...
        const question = this.locale === 'ru' ? 'Удалить неиспользуемый профиль?' : 'Delete the unused profile?';
        if (await vscode.window.showWarningMessage(question, { modal: true }, remove) !== remove) return;
  ...
  html(webview, media) {
    ...
    return `<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
      ...
      <link rel="stylesheet" href="${style}"><title>Symbraid</title></head><body>
      <header><div><h1>Symbraid</h1><p id="projectPath"></p></div><button id="reload" class="secondary" data-i18n="reload"></button></header>
      ...
      <main>
        <section id="tab-overview" class="tab"><div class="cards"><article><h2 data-i18n="activeSource"></h2><dl id="summary"></dl></article><article><h2 data-i18n="serviceStatus"></h2><pre id="statusResult"></pre></article></div></section>
        ...
        <section id="tab-project" class="tab hidden"><h2 data-i18n="projectSettings"></h2><p data-i18n="projectHint"></p><form id="projectForm">
          <label class="check"><input type="checkbox" name="auto_watch"><span data-i18n="autoWatch"></span></label>
          <div class="grid"><label><span data-i18n="backend"></span><select name="backend"><option value="lancedb">LanceDB</option><option value="qdrant">Qdrant</option></select></label>
          <label><span data-i18n="embeddingProfile"></span><select name="embedding_profile" data-profiles></select></label></div>
          <fieldset data-backend="lancedb"><legend>LanceDB</legend><label><span data-i18n="lanceRoot"></span><div class="path"><input name="lancedb_root"><button type="button" data-pick="lancedb_root" data-kind="folder">…</button></div></label></fieldset>
          <fieldset data-backend="qdrant"><legend>Qdrant</legend><div class="grid"><label><span>URL</span><input name="qdrant_url"></label><label><span data-i18n="apiKey"></span><input name="qdrant_api_key" type="password" autocomplete="new-password" placeholder="••••••"></label></div></fieldset>
        ...
        <section id="tab-sources" class="tab hidden"><h2 data-i18n="sources"></h2><table><thead><tr><th>ID</th><th data-i18n="backend"></th><th data-i18n="embeddingProfile"></th><th></th></tr></thead><tbody id="sourceRows"></tbody></table></section>
